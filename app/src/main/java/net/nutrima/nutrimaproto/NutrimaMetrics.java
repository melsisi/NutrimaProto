...
//--- Calculation ---
    ...
    public void calcCalNutrima(UserProfile userProfile) {
        ...
        */
        try {
            if (userProfile.getGender() == Gender.MALE) {
                if (bmi > 25 || bfNutrima > 15) { //TODO: Revisit numbers
                    calNutrima = (int) (weightMainCal - userProfile.getCalorieOffset());
                } else {
                    calNutrima = calUser;
                }
            } else {
                if (bmi > 25 || bfNutrima > 25) { //TODO: Revisit numbers
                    calNutrima = (int) (weightMainCal - userProfile.getCalorieOffset());
                } else {
                    calNutrima = calUser;
                }
    ...
    public void calcBfNutrima(UserProfile userProfile) {
        ...
        try {
            if (userProfile.isAtheletic()) {
                ...
                this.bfNutrima = (this.bfYmca + this.bfCovert) / 2;
            } else {
                calcBfYmcaModified(userProfile);
                calcBfDod(userProfile);
                this.bfNutrima = (this.bfYmcaModified + this.bfDod) / 2;
            }
        } catch (ArithmeticException ae) {
            ae.printStackTrace();
        }
    ...
    public void calcUsdaMacroNutrients (UserProfile userProfile) {
        ...
        usdaProteinMax = (int) ((35 * calNutrima) / (4 * 100)); //--x--

        usdaFatMin = (int) ((20 * calNutrima) / (9 * 100));    //--x--
